...
class Calculator:
    def __init__(self,master):
        ...
        self.eqation = StringVar()
        self.entry_value = ''
        Entry(width=14,bg="#fff",font=('Arial Bold',28),textvariable=self.eqation).place(x=0,y=0)

        Button(width=7,height=3,text='<',relief='flat',bg='white',command=lambda:self.remove()).place(x=300,y=0)
    ...
    def show(self,value):
        if(self.eqation == "Error"):
            self.eqation = ''
        self.entry_value += str(value)
        self.eqation.set(self.entry_value)
    
    def remove(self):
        self.entry_value = self.entry_value[:len(self.entry_value)-1]
        self.eqation.set(self.entry_value)
    ...
    def solve(self):
        try:
            ...
            self.eqation.set(result)
            self.entry_value = str(result)
        except:
            self.entry_value = ''
            self.eqation.set("Error")
